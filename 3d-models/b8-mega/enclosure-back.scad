...
difference() {
    union() {
        ...
        translate([0, display_width_, 0]) {
            cube([display_length, 2 * gap + pcb_padding_y - thickness, thickness]);
            cube([thickness, 2 * gap + pcb_padding_y - thickness, switches_height_]);
            
            translate([0, 2 * gap + pcb_padding_y - thickness, 0]) {
    ...
    translate([0, display_width_ + 2 * gap + pcb_padding_y - thickness, 0]) {
        translate([display_length, -0.1, 0]) // workaround
            rotate([-90, -90, 0])
                screw_base_sub(screw_d, screw_h);
